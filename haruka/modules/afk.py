...
def afk(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text(tld(chat.id, f"{fname} cabut dulu mau buat ngewe!"))
...
def no_longer_afk(bot: Bot, update: Update):
    ...
    if res:
        ...
        try:
            update.effective_message.reply_text(tld(chat.id, f"{firstname} im back abis ngewe nih!"))
        except:
            return
...
def check_afk(bot, update, user_id, fst_name):
    ...
    if sql.is_afk(user_id):
        ...
        if not user.reason:
            res = tld(chat.id, f"{fst_name} is CABUT!")
        else:
            res = tld(chat.id, f"{fst_name} is CABUT! says its because of:\n{user.reason}")
